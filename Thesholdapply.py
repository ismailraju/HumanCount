...
img_pre = image_resize(img_pre, height = 150)
img_pre = image_resize(img_pre, height = 180)
img_pre = cv2.cvtColor(img_pre, cv2.COLOR_BGR2GRAY)


kernel_size = 11
img_pre = cv2.GaussianBlur(img_pre,(kernel_size, kernel_size),1)
#sharp 
#kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
#img_pre = cv2.filter2D(img_pre, -1, kernel)
#edge
low_threshold = 50
high_threshold = 150
img_pre = cv2.Canny(img_pre, low_threshold, high_threshold)
...
while True:
        ret, img = cap.read()
        img = image_resize(img, height = 150)
...
#        images = [img, th1, th2, th3]
        images = [ img_pre-img, th1_sub, th2_sub, th3_sub]
